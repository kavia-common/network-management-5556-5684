...
def create_app():
    ...
    # Read allowlist from env (comma-separated), or use defaults if not set
    allowlist_env = os.getenv("FRONTEND_ORIGIN_ALLOWLIST", "")
    if allowlist_env.strip():
        origins = [o.strip() for o in allowlist_env.split(",") if o.strip()]
    else:
        # Development-safe defaults as documented
        origins = [
            "http://localhost:3000",
            "http://127.0.0.1:3000",
    ...
    # Configure API documentation (preserve existing setup)
...
# For existing imports expecting `app` and `api`, instantiate here to preserve compatibility.
app = create_app()
api = next((v for k, v in globals().items() if isinstance(v, Api)), None)
